...
scalper_strategy_4_t::scalper_strategy_4_t(
...
{
    m_lg.add_attribute("pid", attr::constant<scalper_strategy_4_t*>(this));
...
void scalper_strategy_4_t::order_succeeded(const order_base_t& order)
{
...
void scalper_strategy_4_t::order_failed(const order_base_t& order)
{
...
void scalper_strategy_4_t::order_del_succeeded(const order_base_t& order)
{
...
void scalper_strategy_4_t::order_del_failed(const order_base_t& order)
{
...
void scalper_strategy_4_t::order_move_succeeded(const order_base_t& order)
{
...
void scalper_strategy_4_t::order_move_failed(const order_base_t& order)
{
...
void scalper_strategy_4_t::add_deal(const deal_base_t& deal)
{
    ...
    if ( !trade.is_started() )
    {
        ...
        {
            trade.ask_price( trade.bid_price() + 1*m_symbol.step() );
            trade.bidask = 1;
        ...
        {
            trade.bid_price( trade.ask_price() - 1*m_symbol.step() );
            trade.bidask = -1;
    ...
    if ((trade.buy_volume() == trade.sell_volume()) && (trade.orders.size() == 0))
    {
        ...
        {
            if (trade.bidask == 1)
                m_stat2bid.push_back( Stata2{ trade.start_time(), trade.total()/std::abs(trade.total()) } );
            else
                m_stat2ask.push_back( Stata2{ trade.start_time(), trade.total()/std::abs(trade.total()) } );
        }
        //std::cout << "trade complete" << std::endl;
        //std::cout << boost::posix_time::to_simple_string(trade.start_time()) << "\n";
    }
}

void scalper_strategy_4_t::trade_emplace()
{
    m_trades.emplace_back( m_symbol );
}

void scalper_strategy_4_t::process_trades()
{
...
void scalper_strategy_4_t::add_tick(const deal_base_t& tick)
{
...
void scalper_strategy_4_t::process()
{
...
void scalper_strategy_4_t::update_orders_before()
{
    map<int, int> ob;
...
void scalper_strategy_4_t::update_orderbook()
{
...
void scalper_strategy_4_t::init(scalper_trade_t& trade)
{
...
bool scalper_strategy_4_t::try_to_start(scalper_trade_t& trade)
{
...
void scalper_strategy_4_t::cancel(scalper_trade_t& trade)
{
...
scalper_strategy_4_t::strategy_state_t scalper_strategy_4_t::get_state()
{
...
void scalper_strategy_4_t::work()
{
    ...
    for (scalper_trade_t& trade : m_trades)
    {
        if ( !trade.should_cancel() )
        {
            ...
            else
            {
                ...
                if (m_market.datetime() - trade.start_time() > seconds(1200))
                {
...
void scalper_strategy_4_t::prepare_to_stop()
{
    m_trades.erase(std::remove_if(m_trades.begin(),
                                  m_trades.end(),
                                  [](const scalper_trade_t& trade) {
...
void scalper_strategy_4_t::add_order(scalper_trade_t& trade,
...
bool scalper_strategy_4_t::del_order(scalper_order_t& order)
{
    if (order.is_pending())
        return false;
    order.start_pending(m_market.datetime());
...
bool scalper_strategy_4_t::move_order(scalper_order_t& order, double price)
{
...
tuple<int, int> scalper_strategy_4_t::find_order(int order_id)
{
...
void scalper_strategy_4_t::verify_orders()
{
    for (auto& trade : m_trades)
    {
        auto& orders = trade.orders;
...
int scalper_strategy_4_t::orders_before(const scalper_order_t &order, int n)
{
    if (order.dir() == order_dir_t::bid)
    {
...
int scalper_strategy_4_t::levels_before(const scalper_order_t &order)
{
    if (order.dir() == order_dir_t::bid)
    {
...
int scalper_strategy_4_t::orders_after(const scalper_order_t &order, int n)
{
    if (order.dir() == order_dir_t::bid)
    {
...
int scalper_strategy_4_t::attack_orders(const scalper_order_t &order, int n)
{
    if (order.dir() == order_dir_t::bid)
    {
...
int scalper_strategy_4_t::orderbook_volume(order_dir_t dir, int depth) const
{
    if (depth == 0)
    {
...
int scalper_strategy_4_t::tick_volume(deal_type_t type, const time_duration& duration)
{
    int volume = 0;
...
void scalper_strategy_4_t::check_logger()
{
    ptime now = m_market.datetime();
...
//        m_sink->locked_backend()->add_stream(stream);
        ...
        m_sink->set_filter(
                    expr::attr<scalper_strategy_4_t*>("pid") == this );
...
void scalper_strategy_4_t::write_stat() const
{
//    struct stat_t
//    {
...
void scalper_strategy_4_t::write_stata2() const
{
    ...
    for (const Stata2& stata : m_stat2bid)
    {
        ss << stata.datetime << "," << stata.result << "\n";
        //std::cout << stata.datetime << "," << stata.result << "\n";
        //std::cout << boost::posix_time::to_simple_string(ptime) << "\n";
    }
    std::ofstream file_bid{"stata2bid.csv"};
    file_bid << ss.rdbuf();

    std::stringstream ss1;
    ss1.imbue(std::locale(ss.getloc(), facet));

    for (const Stata2& stata : m_stat2ask)
    {
        ss1 << stata.datetime << "," << stata.result << "\n";
    }
    std::ofstream file_ask{"stata2ask.csv"};
    file_ask << ss1.rdbuf();
}
